...
>     如果gets()函数读入的字符数超过了字符数组的界的时候，会发生什么呢？

&emsp;&emsp;我在昨天整理以前的C语言笔记的时候刚刚好试验了一下，试验的过程如下：
...
&emsp;&emsp;今天看CSAPP的时候刚好看到差不多的内容，并且作者指出这种越界问题会导致缓冲区溢出，有许多病毒就是利用缓冲区溢出进行攻击的。

&emsp;&emsp;所以今天想好好思考思考这个问题。
...
>     这种方法属于更大的一类技术，这类技术叫ASLR（地址空间布局随机化）。但是也有办法对付ASLR，比如一种操作叫[nop sled](https://en.wikipedia.org/wiki/NOP_slide).

## 栈破坏检测

&emsp;&emsp;GCC在产生的代码中加入了一种机制来检测缓冲区越界，其思想也很简单，就是在局部缓冲区和状态区之间放一个哨兵值（不知道为什么叫`canary`,金丝雀😅），这个是随机产生的。
